...
function getAllHouses()
{
	...
	// Agent can only view their houses
...
function createHouse($request_values)
	{
		global $conn, $errors, $title, $featured_image, $floor_id, $body, $published;
		...
		if (count($errors) == 0) {
			$query = "INSERT INTO houses (user_id, title, slug, image, body, published, created_at, updated_at) VALUES(1, '$title', '$house_slug', '$featured_image', '$body', $published, now(), now())";
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	{
		global $conn, $title, $house_slug, $body, $published, $isEditingHouse, $house_id;
	...
	{
		global $conn, $errors, $house_id, $title, $featured_image, $floor_id, $body, $published;
		...
		if (count($errors) == 0) {
			$query = "UPDATE houses SET title='$title', slug='$house_slug', views=0, image='$featured_image', body='$body', published=$published, updated_at=now() WHERE id=$house_id";
