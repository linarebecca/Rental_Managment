...
<?php 
	...
	if(isset($_POST['mpesa_btn'])) {
		...
		$query = "INSERT INTO house_deposit_tenant(user_id,house_id,deposit_amount,mode_of_pay,pay_code,created_at) ";
		$query .= "VALUES('{$user_id}','{$house_id}','{$deposit_amount}','{$mode_of_pay}','{$mpesa_code}',now()) ";
		$create_query = mysqli_query($conn,$query);
		$house_det = isset($_SESSION['deposit']);
		echo var_dump($house_det);
	
		if (!$create_query) {
		die('QUERY FAILED' . mysqli_error($conn));
		}
		header("Location: tenant_invoice.php?user_id=$user_id&house_id=$house_det");
	...
	if(isset($_POST['card_btn'])) {
		...
		$query = "INSERT INTO house_deposit_tenant(user_id,house_id,deposit_amount,mode_of_pay,pay_code,created_at) ";
		$query .= "VALUES('{$user_id}','{$house_id}','{$deposit_amount}','{$mode_of_pay}','{$card_code}',now()) ";
		$create_query = mysqli_query($conn,$query);
		$house_det = isset($_SESSION['deposit']);
		echo var_dump($house_det);
	...
	if(isset($_POST['cash_btn'])) {
		...
		$query = "INSERT INTO house_deposit_tenant(user_id,house_id,deposit_amount,mode_of_pay,pay_code,created_at) ";
		$query .= "VALUES('{$user_id}','{$house_id}','{$deposit_amount}','{$mode_of_pay}','{$cash_code}',now()) ";
		$create_query = mysqli_query($conn,$query);
		$house_det = isset($_SESSION['deposit']);
		echo var_dump($house_det);
...
<div class="container">
	...
	<div style="width: 40%; margin: 20px auto;">
		<form method="post" action="deposit_house.php" >
			<h2>DEPOSIT PAYMENT & Tenant Clarification</h2>
			...
			<input type="hidden" name="house_id" value="<?php echo $house['id']; ?>" placeholder="">
